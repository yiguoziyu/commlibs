package com.ljj.commonlib.helper
...
object ToastHelper {
     ...
     fun showToast(mContent: String) {
        val mContext = ContextProvider.get().application
         val toast = Toast.makeText(mContext, mContent, Toast.LENGTH_SHORT)
        toast.setGravity(Gravity.CENTER, 0, 0)//设置位置
        val ll = LayoutInflater.from(mContext).inflate(R.layout.toast_style, null)
        val tx = ll.findViewById<TextView>(R.id.toast_content)
        tx.text = mContent
        toast.view = ll//设置外观
        toast.show()
    }
    fun showLongToast(mContent: String) {
        val mContext = ContextProvider.get().application
        val toast = Toast.makeText(mContext, mContent, Toast.LENGTH_LONG)
        toast.setGravity(Gravity.CENTER, 0, 0)//设置位置
        val ll = LayoutInflater.from(mContext).inflate(R.layout.toast_style, null)
        val tx = ll.findViewById<TextView>(R.id.toast_content)
        tx.text = mContent
